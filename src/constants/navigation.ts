...
export interface NavItem {
  ...
  children?: NavChild[];
  /**
   * رابط quick-add اختياري — يظهر كزرار "+" صغير بجانب الـ item
   * نفسه (لو عنده children). يفيد للحالات اللي المستخدم عادة
   * بيحتاج فيها action سريع بدون فتح القائمة الفرعية كاملة.
   * مثال: "الجلسات" → quick-add لـ "جلسة جديدة" مباشرة.
   */
...

  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: Megaphone,
        children: [
          { label: "كل الجلسات", href: "/dashboard/sessions" },
          { label: "جلسة جديدة", href: "/dashboard/sessions/new" },
        ],
      },
    ],
  },
