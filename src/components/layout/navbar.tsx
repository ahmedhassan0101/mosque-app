...
// /*
//   Navbar — Shell Structure Only
//   ─────────────────────────────────────────────
//   h-14 (56px) — نفس ارتفاع الـ Sidebar header، عشان الخط الفاصل
//   الأفقي والعمودي يتقابلوا بصرياً عند نقطة واحدة (top-start corner).

//   Layout:
//     [Mobile Trigger]   [spacer flex-1]   [Actions: theme + avatar]

//   على lg+: الـ MobileSidebarTrigger بيختفي تلقائياً (هيتحدد جواه)
//   لأن الـ Sidebar الثابت already ظاهر.
//   ─────────────────────────────────────────────
// */
...
//       {/* Mobile trigger — يحمل Sidebar جوه Sheet، مختفي على lg+ */}
//       <MobileSidebarTrigger role={role} />

//       {/* Spacer — يدفع الـ actions لآخر الصف (يسار في RTL) */}
//       <div className="flex-1" />

//       {/* Actions cluster — placeholder، التفاصيل لاحقاً */}
...
/*
  Navbar — Final Assembly
  ─────────────────────────────────────────────
  Layout:
    [Mobile Trigger]  [Search]   [spacer flex-1]   [Theme][separator][Avatar]

  h-14 — يطابق ارتفاع الـ Sidebar header بالضبط (راجع Sidebar.tsx)
  عشان الخط الفاصل العمودي والأفقي يتقابلوا بصرياً في نقطة واحدة.

  MobileSidebarTrigger بيختفي تلقائياً على lg+ (jواه className
  الخاصة بيه: "lg:hidden") لأن الـ Sidebar الثابت already ظاهر.
  ─────────────────────────────────────────────
*/
...
export function Navbar({ session }: NavbarProps) {
  ...
  return (
    <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border bg-background px-4 md:px-6">
      {/* Mobile trigger — مختفي تلقائياً على lg+ */}
      <MobileSidebarTrigger role={role} />

      {/* Search — متاح من أي مكان عبر Cmd+K */}
      <SearchDialog role={role} />

      {/* Spacer — يدفع الـ actions لآخر الصف (يسار بصرياً في RTL) */}
      <div className="flex-1" />
