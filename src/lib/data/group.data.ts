...
export const getGroupsList = cache(
  async (type: ActivityType): Promise<GroupListItem[]> => {
    try {
      ...
      // After populate, teacherId becomes the populated Teacher object (or null if deleted)
      // type PopulatedGroup = Omit
      //   Awaited<ReturnType<typeof Group.findOne>> extends null ? never : NonNullable<Awaited<ReturnType<typeof Group.findOne>>>,
      //   "teacherId"
      // > & {
      //   _id: unknown;
      //   mosqueId: unknown;
      ...
      //    type Omit<T, K extends keyof any> = { [P in keyof T extends K ? never : keyof T]: T[P]; }
